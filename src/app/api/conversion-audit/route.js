...
function analyze(html) {
  ...
  if (ctaHits.length) add('ok', `PRIMARY CTA â€” ${ctaHits.length} ACTION BUTTON(S)`);
  else if (ctaInText) { add('warn', 'CTA TEXT PRESENT â€” NO CLEAR BUTTON'); issue('warn', 'No prominent call-to-action button', 'The page mentions an action but has no obvious CTA button. Visitors ready to convert have nothing clear to click. Add a high-contrast primary button above the fold.'); }
  else { add('risk', 'NO CALL-TO-ACTION DETECTED'); issue('risk', 'No call-to-action', "There's no detectable CTA â€” button or phrase â€” telling visitors what to do next. Traffic with nowhere to go doesn't convert. Add one unmistakable primary action (Get a quote, Book a call).") ; }
  ...
  if (contactSignals.length) add('ok', `CONTACT â€” ${contactSignals.join(', ').toUpperCase()}`);
  ...
  if (forms) add('ok', `FORMS â€” ${forms} ON PAGE`);
  ...
  if (reviewSchema) add('ok', 'TRUST â€” REVIEW/RATING SCHEMA');
  if (trustText || reviewSchema) {
    if (!reviewSchema) add('ok', 'TRUST â€” SOCIAL-PROOF LANGUAGE PRESENT');
  } else {
  ...
  if (socials.length) add('ok', `SOCIAL â€” ${socials.length} PROFILE LINK(S)`);
  ...
  else add('ok', 'VALUE PROP â€” H1 PRESENT');
  ...
  const verdict = risks > 0 ? 'CONVERSION PATH BROKEN â€” TRAFFIC HAS NOWHERE TO GO'
    : warns > 0 ? 'CONVERTS, BUT LEAKS â€” TRUST & PATH GAPS COST YOU LEADS'
    : 'CONVERSION FUNDAMENTALS â€” IN PLACE';
...
export async function POST(request) {
  if (rateLimited(clientIp(request), 10, 'conversion-audit')) {
